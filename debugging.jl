...
if length(ARGS) != 3
    throw(InvalidARGS("Required julia debugging.jl <camera_type> <angle_z> <angle_y>     <camera_type>: perspective or orthogonal    <angle_z>: rotation around z axis (in deg)     <angle_y>: rotation around z axis (in deg)"))
end

if ARGS[1] == "perspective"
    path = "./demo_path/"
    pfm_filename_and_path = "./demo_path/debugging_perspective_z" * ARGS[2] * "_y" * ARGS[3] * ".pfm"
    filename = "debugging_perspective_z" * ARGS[2] * "_y" * ARGS[3]
...
elseif ARGS[1] == "orthogonal"
    path = "./demo_path/"
    pfm_filename_and_path = "./demo_path/debugging_orthogonal_z" * ARGS[2] * "_y" * ARGS[3] * ".pfm"
    filename = "debugging_orthogonal_z" * ARGS[2] * "_y" * ARGS[3]
    angle_z = parse(Float64, ARGS[2])
    angle_y = parse(Float64, ARGS[3])
    rot1 = rotation("z", angle_z*π/180.0)
    rot2 = rotation("y", angle_y*π/180.0)
...
s1 = Sphere(traslation(Vec(0.0, 0.0, 0.0))(scaling(0.3)), Material(DiffuseBRDF(UniformPigment(RGB(1.0, 1.0, 0.0)))))
...
RND = PathTracer(w, RGB(0.0, 0.0, 0.0), pcg, 1, 0, 2)

#fire_all_rays!(IT, RND)

hit_ray = fire_ray(IT, 1, 900)
color = RND(hit_ray)
